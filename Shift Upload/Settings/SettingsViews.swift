        ...
        VStack(alignment: .leading, spacing: 18) {
            ...
            ShiftHubMacSettingsSectionCard {
                ...
                ShiftHubMacAboutFeatureRow(
                    ...
                    detail: localized("PDFのみ対応しています。文字を選択・コピーできる文字データ層を持ち、左側に職員名、右側に1日から月末までの日付が横方向に並ぶ勤務表を使用してください。セルを区切る罫線がPDF内の図形情報として保存されている形式が対象です。画像だけのスキャンPDFや縦向きPDFには対応していません。")
                )
                Divider()
                ShiftHubMacAboutFeatureRow(
    ...
    var body: some View {
        List {
            ...
            Section("対応形式と注意点") {
                ShiftHubAboutRow(
                    title: "PDF入力条件",
                    detail: "PDFのみ対応しています。文字を選択・コピーできる文字データ層を持ち、左側に職員名、右側に1日から月末までの日付が横方向に並ぶ勤務表を使用してください。セルを区切る罫線がPDF内の図形情報として保存されている形式が対象です。画像だけのスキャンPDFや縦向きPDFには対応していません。"
                )
                ShiftHubAboutRow(
                    title: "年月の判定",
